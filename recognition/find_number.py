...
class my_recognizer():
    def start():
        ...
        def work(digit_img):
            ...
            try:
                npaClassifications = np.loadtxt("classifications_for_questions.txt", np.float32)                  # read in training classifications
            except:
            ...
            try:
                npaFlattenedImages = np.loadtxt("flattened_images_for_questions.txt", np.float32)                 # read in training images
            except:
        ...
        #====================================================================================================
        dir = os.listdir('recognition/temp_photos/cropped')
        try:
        	for i in dir:
        		digit = cv2.imread(str('recognition/temp_photos/cropped/'+str(i)))
        		digit = digit[14:68,14:68]
        		num1 = work(digit)
        		if num1 == 20:
        			num1 = 10
        		...
        		mdx = str(i)
        		mdx = mdx.replace(".png","")
        		cv2.imwrite(str('recognition/temp_photos/arranged/'+str(mdx)+'_'+str(num1)+'.png'), digit)
        		#os.system('pause')
